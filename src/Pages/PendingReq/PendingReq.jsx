    ...
    async function fetchPendingRequests() {
      ...
      try {
        const response = await axios.get(
          "http://192.168.137.1:8080/api/requests/get/pending",
          {
            headers: {
    ...
    switch (normalizeStatus(status)) {
      case "PENDING":
        return (
          <FontAwesomeIcon
            icon={faClock}
            className="icon-legend__icon--pending"
          />
        );
      case "APPROVED":
        return (
          <FontAwesomeIcon
            icon={faCheckCircle}
            className="icon-legend__icon--approved"
          />
        );
      case "CANCELED":
        return (
          <FontAwesomeIcon
            ...
            className="icon-legend__icon--canceled"
          />
        );
      case "COMMENTED":
        return (
          <FontAwesomeIcon
            icon={faCommentDots}
            className="icon-legend__icon--commented"
          />
        );
      default:
        return null;
  ...
  return (
    <>
      <div className="pending-req-container">
        <div className="title-instuctions">
          {" "}
          <h2>Pending Requests</h2>
          <div className="icon-legend-penReq">
            <div className="icon-legend__item">
              <FontAwesomeIcon
                icon={faClock}
                className="icon-legend__icon--pending"
              />
              <span className="icon-legend__text">PENDING</span>
            </div>
            <div className="icon-legend__item">
              <FontAwesomeIcon
                icon={faCheckCircle}
                className="icon-legend__icon--approved"
              />
              <span className="icon-legend__text">APPROVED</span>
            </div>
            <div className="icon-legend__item">
              <FontAwesomeIcon
                icon={faExclamationCircle}
                className="icon-legend__icon--canceled"
              />
              <span className="icon-legend__text">CANCELED</span>
            </div>
           
          </div>
        </div>

        <table className="pending-req-table">
          <thead>
            <tr>
              <th>Req ID</th>
              <th>Request Name</th>
